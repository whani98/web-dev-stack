...
CREATE TABLE HRM_LEAVES (
   ...
   START_END DATE, -- 연차 시작
   END_DATE DATE, -- 연차 마지막
   REASON TEXT,
   STATUS VARCHAR(30) CHECK(STATUS IN ('신청', '승인', '반려')) DEFAULT '신청',
   CREATE_DATE DATETIME DEFAULT (CURRENT_DATE), -- 신청 날짜check
   USER_NO INT
);
-- 평가
...
CREATE TABLE HRM_WORK (
   ...
   WORK_START DATETIME DEFAULT (CUREENT_DATE),
   WORK_END DATETIME,
...
FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);
-- HRN_EVALUATION : USER_NO
ALTER TABLE HRN_EVALUATION ADD
FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);
-- HRM_WORK : USER_NO
...
FOREIGN KEY (DEPT_NO) REFERENCES DEPARTMENT(DEPT_NO);
-- FIANANCAIL_TRANSACTION : USER_NO
ALTER TABLE FIANANCAIL_TRANSACTION ADD
FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);
-- QUALITY : USER_NO
...
FOREIGN KEY (USER_NO) REFERENCES QUALITY(QUALITY_NO);
ALTER TABLE QUALITY_DEFECT ADD
FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);