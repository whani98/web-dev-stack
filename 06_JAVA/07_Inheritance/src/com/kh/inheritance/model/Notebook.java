 ...
 *  - 명시되지 않아도 모든 클래스는 Object 클래스 상속 -> Object 클래스의 메서드들을 오버라이딩 하여 메서드 재정의 가능
 *  - 부모 클래스의 생성자, 초기화 블록은 상속되지 않는다. -> 자식 클래스 생성 시 부모 클래스 생성자가 먼저 실행
 *  - 부모의 private 변수는 상속은 되지만 직접 접근은 불가능 ->  Getter / Setter 사용해서 간접 접근 가능
 *  
 * */

public class Notebook extends Product { // 반드시 부모(extends 뒤에 오는 것)는 하나만!
