...
public class Create {
	...
	public void method1() {
		...
		// rangeClosed : 첫번째 파라미터값 ~ 두번째 파라미터값까지의 값을 요소로 가지는 스트림 객체 생성
	...
	public void method2() {
		...
		stream.forEach(name -> System.out.println(name)); // 반복문 쓰는 것과 같이 세로로 나열됨
//		stream.forEach(name -> System.out.println(name)); // 한 번 더 작성하면 오류남
		
		List<String> nameList = Arrays.asList(names);
		stream = nameList.stream();
		stream.forEach(name -> System.out.println(name)); // 반복문
	}
