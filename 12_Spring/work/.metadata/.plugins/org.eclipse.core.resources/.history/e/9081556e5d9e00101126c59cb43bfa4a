package controller;

import java.util.List;

import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import dao.MemberDAO;
import util.PwdBcrypt;
import vo.MemberVO;

@Controller
public class MemberController {

	MemberDAO member_dao;

	public void setMember_dao(MemberDAO member_dao) {
		this.member_dao = member_dao;
	}

	// 회원 조회
	@RequestMapping("/list.do")
	public String list(Model model) {

		List<MemberVO> list = member_dao.selectList();

		// 바인딩
		model.addAttribute("list", list);

		// 포워딩
		return "member/member_list";
	}

	// 회원 등록 페이지
	@RequestMapping("/member_register_form.do")
	public String registerIn() {

		return "member/member_register";
	}

	// 회원 등록
	@RequestMapping("/member_register.do")
	public String register(MemberVO vo) {

		// 비밀번호 암호화
		PwdBcrypt pbc = new PwdBcrypt(); // 암호화용 객체 생성
		String encodePwd = pbc.encodingPwd(vo.getPwd()); // 입력한 비밀번호 암호화하여 encodePwd에 저장

		vo.setPwd(encodePwd); // 암호화한 비밀번호 vo에 set

		int res = member_dao.insert(vo);

		return "redirect:list.do";
	}

	// 회원 삭제
	@RequestMapping("/member_delete.do")
	@ResponseBody
	public String delete(int idx) {

		int res = member_dao.delete(idx);

		String result = "no";

		// 삭제 성공 시 res == 1, 삭제 실패 시 res == 0
		if (res == 1) {
			result = "yes"; // 삭제 성공 시

			return result;
		}
		return result;
	}

	// 수정할 회원 조회 페이지
	@RequestMapping("/member_one.do")
	public String selectOne(int idx, Model model) {

		MemberVO vo = member_dao.selectOne(idx);

		// 바인딩
		model.addAttribute("vo", vo);

		// 포워딩
		return "member/member_modify";
	}

	// 회원 수정
	@RequestMapping("/member_modify.do")
	public String modify(MemberVO vo) {

		// 비밀번호 암호화
		PwdBcrypt pbc = new PwdBcrypt(); // 암호화용 객체 생성
		String encodePwd = pbc.encodingPwd(vo.getPwd()); // 입력한 비밀번호 암호화하여 encodePwd에 저장

		vo.setPwd(encodePwd); // 암호화된 비밀번호 vo에 set

		int res = member_dao.update(vo);

		return "redirect:list.do";

	}

}
